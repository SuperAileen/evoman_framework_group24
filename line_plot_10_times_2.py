...
def run_optimizer(method):
    cwd = os.getcwd()
    if method == 1:
        result = subprocess.run(['python', 'training_specialist.py'], cwd=cwd, capture_output=True, text=True)
    else:
...
def aggregate_stats(runs, method):
    all_data = []
    for _ in range(runs):
        stats_path = run_optimizer(method)
...
def plot_aggregated_stats(aggregated_data, num_runs, method, enemy):
    ...
    plt.title(f'Method {method}, Enemy {enemy}', fontsize=20)
    plt.xlabel('Generation', fontsize=16)
    plt.ylabel('Fitness', fontsize=16)
    plt.legend()
    plt.grid(True)
    plt.xlim(0, max(generations))
    plt.savefig(f'fitness_method{method}_enemy{enemy}.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    method = 2
    enemy = 8
    aggregated_data = aggregate_stats(num_runs, method)
    plot_aggregated_stats(aggregated_data, num_runs, method, enemy)
